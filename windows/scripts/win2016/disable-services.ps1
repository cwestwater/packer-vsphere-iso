...
if ( $osVersion -eq "Server" )
{
    ...
    Get-Service -Name XblAuthManager | Set-Service -StartupType Disabled | Out-Null

    Write-Host "Disable Xbox Live Game Save"
    Get-Service -Name XblGameSave | Set-Service -StartupType Disabled | Out-Null
    ...
    Get-ScheduledTask -TaskName XblGameSaveTaskLogon | Disable-ScheduledTask | Out-Null
}
else
{
    exit 0
